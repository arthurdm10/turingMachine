        ...
        if(funcs.size() > 0){
            ...
            while(it != funcs.end()){
              //  getchar();
                ...
                auto itInstruction = funcs.equal_range(step);

                for(auto mmIt = itInstruction.first; mmIt != itInstruction.second; mmIt++){
                    std::string function = mmIt->second;
